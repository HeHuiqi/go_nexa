...
const (
	P2PKH                AddressType = 0
	P2SH                 AddressType = 1
	SCRIPT_TEMPLATE_TYPE AddressType = 19
)
...
func DecodeCashAddress(str string) (string, data, error) {
	...
	// println("DecodeCashAddress-prefix:", prefix)
	// println("DecodeCashAddress-values:", hex.EncodeToString(values))
	// Verify the checksum.
...
func CheckDecodeCashAddress(input string) (result []byte, prefix string, t AddressType, err error) {
	...
	// println("CheckDecodeCashAddress-convertBits - ", hex.EncodeToString(data))
	// println("CheckDecodeCashAddress-len(data) - ", len(data))
	...
	case 0x08:
		t = P2SH
	case 0x98:
		t = SCRIPT_TEMPLATE_TYPE
		ret = data[1:25]
	}

	// return data[1:21], prefix, t, nil
	return ret, prefix, t, nil
}
...
func packAddressData(addrType AddressType, addrHash data) (data, error) {
	...
	if addrType != P2PKH && addrType != P2SH && addrType != SCRIPT_TEMPLATE_TYPE {
		return data{}, errors.New("invalid addrtype")
	}
	versionByte := uint(addrType) << 3
	println("versionByte:", versionByte)

	encodedSize := (uint(len(addrHash)) - 20) / 4
	if addrType == SCRIPT_TEMPLATE_TYPE {
		encodedSize = (uint(len(addrHash)) - 21) / 4
	}
	...
	println("encodedSize:", encodedSize)

	println("versionByte2:", versionByte)
